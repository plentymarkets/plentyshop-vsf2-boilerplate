<template>
  <client-only>
    <div>
        ...
        <div class="bg-sf-c-light-lighten shadow-xl p-xs">
          <div v-if="!furtherSettingsOn">
            ...
            <div v-if="cookieJson" class="flex flex-col md:flex-row flex-wrap md:flex-nowrap ml-sf-xs justify-around">
              <div v-for="(cookieGroup, index) in cookieJson" :key="index" class="mr-sf-sm">
                <SfCheckbox v-model="cookieGroup.accepted" v-e2e="`checkbox-${index}`"
                  :disabled="index === defaultCheckboxIndex" :name="cookieBarGetters.getCookieGroupName(cookieGroup)"
                  :label="cookieBarGetters.getCookieGroupName(cookieGroup)" @change="
                    ($event) => setChildrenCheckboxes(cookieGroup, $event)
                  " />
              </div>
            </div>
          </div>
          <div v-else class="overflow-y-auto h-80 bg-sf-c-light-primary-lighten' p-2">
            <div class="h-64">
              <div v-for="(cookieGroup, groupIndex) in cookieJson" :key="groupIndex"
                class="bg-sf-c-light-primary mt-sf-xs p-sf-xs">
                <div>
                  <SfCheckbox v-model="cookieGroup.accepted" :disabled="groupIndex === defaultCheckboxIndex"
                  ...
                  <div v-if="cookieBarGetters.getShowMore(cookieGroup)" class="ml-sf-xs font-sf-light">
                    <div v-for="(
                          cookie, cookieIndex
                        ) in cookieBarGetters.getCookiesList(cookieGroup)" :key="cookieIndex" class="p-sf-sm">
                      <SfCheckbox v-model="cookie.accepted" :disabled="groupIndex === defaultCheckboxIndex"
                        :name="cookieBarGetters.getCookieName(cookie)" :label="cookieBarGetters.getCookieName(cookie)"
                      ...
                      <div v-for="propKey in Object.keys(cookie)" :key="propKey">
                        <div class="flex w-full mb-sf-xs p-sf-xs bg-white">
                          <div v-if="propKey !== 'name'" class="w-1/4">
                            {{ propKey }}
                          </div>
                          <div class="w-3/4">
                ...
                <div class="ml-sf-xs">
                  <SfButton v-if="!cookieBarGetters.getShowMore(cookieGroup)" class="sf-button--text mb-xs"
          ...
          <div class="w-full flex flex-col md:flex-row">
            <div class="flex-1 pr-sf-xs pl-sf-xs text-center z-100000000 mb-sf-xs">
              <button v-e2e="'accept-all'" class="color-primary w-full sf-button" :aria-disabled="false" type="button"
                aria-label="button" @click="convertAndSaveCookies(true, true)">
                {{ $t('CookieBar.Accept All') }}
              </button>
            </div>
            <div class="flex-1 pr-sf-xs pl-sf-xs text-center z-100000000 mb-sf-xs">
              <button v-e2e="'reject-all'" class="color-primary w-full sf-button" :aria-disabled="false" type="button"
                aria-label="button" @click="convertAndSaveCookies(true, false)">
                {{ $t('CookieBar.Reject All') }}
              </button>
            </div>
            <div class="flex-1 pr-sf-xs pl-sf-xs text-center z-100000000 mb-sf-xs">
              <button v-e2e="'accept-selection'" class="sf-button w-full border-solid border-1 border-sf-c-primary"
                :aria-disabled="false" type="button" @click="convertAndSaveCookies(false, false)">
      ...
      <button v-else v-e2e="'cookie-show-banner-button'"
        class="color-primary sf-button z-100 fixed bottom-100 md:bottom-0 right-sf-xs" aria-label="Cookie control"
